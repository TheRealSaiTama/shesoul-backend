...
@Transactional
    public void patchProfileBasic(Long userId, com.sheandsoul.v1update.dto.ProfilePatchRequest patch) {
        Profile profile = profileRepository.findByUserId(userId)
                .orElseThrow(() -> new IllegalArgumentException("Profile not found for user ID: " + userId));

        if (patch.getAge() != null) profile.setAge(patch.getAge());
        if (patch.getHeight() != null) profile.setHeight(patch.getHeight());
        if (patch.getWeight() != null) profile.setWeight(patch.getWeight());
        if (patch.getName() != null) profile.setName(patch.getName());
        if (patch.getNickName() != null) profile.setNickName(patch.getNickName());
